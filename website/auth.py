...
def sign_up():
    if request.method == 'POST' :
        ...
        if user:
            flash('Email id exist', category='error')
        ...
        if len(email) < 4 :
            ...
            flash('Password should be of minimum 8 characters', category='error')
        else:
            new_user = User(email=email, firstname=firstname, password=generate_password_hash(pass1, method='pbkdf2:sha256'))
